...
// icons
import {SiBag, SlBag} from 'react-icons/sl';
  ...
  return (
    ...
    <div className='container mx-auto'>
      <div className='flex flex-row gap-4 lg:items-center justify-between mb-4 xl:mb-0'>
        ...
        {/* category nav mobile */}
        <div className={`${catNavMobile ? 'left-0' : '-left-full'} fixed top-0 bottom-0 z-30
         w-full h-screen transition-all duration-200`}>
        ...
        <div className='flex items-center gap-x-[10px]'>
           {/* phone */}
          <div className='ml-8 mr-8'>Besoin d'aide ? 0641039101</div>
          {/* login */}
        <Link to={'/login'}>
        <button className='mr-9'>Se connecter</button>
        </Link>
           {/* cart icon*/}
          <div onClick={()=> setIsOpen(!isOpen)} className='relative cursor-pointer'>
            <SlBag className='text-2x1' />
            {/* amount */}
            <div className='bg-accent text-primary absolute w-[18px] h-[18px] rounded-full top-3 -right-1 text-[13px]
