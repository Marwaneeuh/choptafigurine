    ...
    <Link to={`/product/${product.id}`}>
     <div className='grad w-full h-[362px] rounded-[8px] overflow-hidden relative group '>
      ...
      {product.attributes.isNew ? (
          <div className='absolute bg-accent text-primary text-[12px] font-extrabold uppercase top-4 right-4 px-2 rounded-full z-10'>nouveauté</div>
      ) : ( 
        ''
      )}
    
      {/* image */}
      <div className='w-full h-[210px] w-[350px] flex items-center justify-center relative'>
        <img 
